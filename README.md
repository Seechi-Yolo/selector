...
`make build` is equivalent to `npm run build` (TypeScript + Vite for the service worker, one IIFE content bundle for the editor, then extension pages). **`make dev` / `npm run dev`** runs a full build once, then watches the service worker, the content script bundle, and the tutorial bundle so `dist/src/pages/**` (tutorial, sandbox, design-system, help-hub) is not wiped while the main bundles are watching.
