...
| **编辑说明**（控件） | **编辑说明**；与 **Enter 开说明**（**D-28**）为同一打开路径的等价表述 | Edit instruction |
| **Enter 开说明** | 页上 **Enter** 角标提示或引导中的 **Enter** | Open instruction via Enter |
...
| **Enter 开说明** / **编辑说明** | **Enter** 提示为主（**D-20**）；与「编辑说明」文案等价 | **D-20** 角标：逐项在各元素轮廓约定角；多选在 **D-17** 包络上；展示 **Enter** 以提示键盘路径 |
...
- **呈现**：**整体修改说明** → 界面「**对当前选取的说明**」；**逐项修改说明** → 「**修改说明**」。说明层默认不展示；用户按 **Enter**（**D-28**）或点等价角标后进入 **O**。**D-18** / **D-23**：无「完成」「清除」「关闭」控件；说明层内 **Enter 提交 · Shift+Enter 换行** 等短提示在输入层内展示（**不**展示 Ctrl+Delete 清空正文句），主面板仅保留极简状态句。
...
**自上而下、由内到外速览**：**选取会话面板**（**D-27** 极小）→ **提示 / 当前选中** Tab（**D-26**）→ 画布描边、**范围选取**、**多选主框**、**Enter** 提示角标（**D-20** / **D-28**）、暂停态 → **Enter** 后选取框**正下方** **说明面板**（**D-22**）。
...
| D-12 | 多选形成期面板 | 框选拖移、**Shift+点击** 追加过程中**不**打开 **O**（保持 **N**）。Shift 链结束：**最后一次**选中变更起 **500 ms** 无新变化；框选以**松手**为准；**≥2** 项后进入 **D-13**（仍为 **N**，待 **Enter** 开整体，**D-28**）。 |
| D-13 | 两层说明模型 | 选取建立或变更焦点后，说明浮层默认 **N**（**D-28**）。**≥2** 项：形成期结束且整体闸未放行时，用户按 **Enter** 打开**整体修改说明**（**D-17** 主框语境下 **O**）；整体提交放行后，再经 **D-20** 入口进入各元素**逐项**（仅该项）。**仅 1 项**：不经整体层；用户按 **Enter** 打开**逐项 O**。由 1 项增为多项时，**D-12** 结束后同样先整体再逐项；此前逐项与整体的合并/清空由实现与导出模板对齐。 |
| D-28 | 说明层打开 | 已选且说明关 **N**、点选未暂停 **A**、焦点不在宿主原生可编辑区且非扩展 UI 内时，**Enter**（无 Shift / ⌘ / Ctrl / Alt）打开说明浮层：**≥2 且整体闸未放行** → **整体 O**；否则 → **逐项 O**。**P**（暂停）下 **Enter** 不打开说明（先 **Space** 回 **A**）。与点「编辑说明」角标等价。 |
...
| D-20 | 角标位置与 **Enter** 提示 | **逐项**：各元素轮廓约定角，展示 **Enter** 以提示「开说明」路径（可与「编辑说明」文案择一或并存）。**多选/框选**：**D-17** 包络（可与 **D-01** 矩形合一）上**另设**角标；与逐项并存时不挡关键操作。 |
...
| D-23 | 说明编辑默认逻辑 | **说明层已开（O）** 且焦点在说明输入框内时：**Enter**（不含 Shift）提交——整体闸未放行时等同**完成整体说明**并关闭；其余为**关闭说明层**（草稿随输入实时写入）；提交后宿主可立刻再刷剪贴板并给出已复制反馈（与 **D-08** 防抖互补）。**Delete / Backspace** 逐字删；**Ctrl+Delete** / **⌘+Delete** / **⌘+Backspace** 仍清空该层正文（**不**在输入层提示行展示该快捷键）。**Enter 提交 · Shift+Enter 换行** 的短提示置于**输入浮动层内**（输入框下沿一行），**「提示」** Tab **不**重复长句。扩展 UI 外 **pointerdown** 关闭说明层等仍适用。**Enter 开说明**见 **D-28**（宿主侧：说明关时同一键打开浮层）。 |
| D-24 | 操作引导信息量 | **少露字**：主引导仍以一句为主（**D-09**）；辅句**更少、更短**；**D-15** 剪贴板辅句门闩不变。 |
| D-25 | 头部 icon 按钮 | 最小化、关闭为**小尺寸** icon（相对上一版「加大」决策废止），与 **D-27** 极小面板匹配。 |
| D-26 | 主面板视图切换 | **「提示」**（默认：操作引导 + 吐司）与 **「当前选中」**（已选列表）二选一展示；标题栏与关闭同排 **单键**（左右箭头图标）循环切换。最小化时整面板缩为右侧竖条并 **磁吸** `right:6px`（浮动布局），展开恢复右下默认位。 |
...
- **说明与 Esc**：**D-12**、**D-13**、**D-17**、**D-18**、**D-23**、**D-28**、**D-11**（多选先整体后逐项；选取后默认 **N**，**Enter** 或角标开 **O**；改选焦点回 **N**）。
...
3. **单选路径**（首次唯一单击或范围仅一项）：建立选取后为 **H+A+N**（**D-28**）；用户按 **Enter**（或点等价角标）后进入 **逐项 O**（**D-13**）。
4. **多选路径**（范围 **≥2** 松手，或 Shift 链结束 **≥2**）：形成期保持 **N**、不打开 **O**（**D-12**）；完成后为 **H+A+N**，用户按 **Enter** 打开**一次** **整体 O**（**D-17** 主框）；整体放行后经 **D-20** 入口写**逐项**（仅该项）。全程仅增至一项则同单选（**Enter** 后逐项 **O**）。
5. **改选焦点**：说明默认回到 **N**；用户再按 **Enter** 或点 **编辑说明** / **Enter** 角标进入 **O**（**D-28**）。
6. **已选中引导**：**D-09** + **D-15**（复制辅助条默认隐藏至成功 **⌘/Ctrl+C** 后）。
7. **页上描边与角标**：浅绿可选取 / 灰暂停（**D-02**、**D-19**）；范围无填充（**D-01**）；**Enter** 提示与角标 **D-20**；去掉选取项 **D-30**（页上控件若保留则 **D-05**/**D-21**）；**Enter** / **编辑说明** → 开 **O** 后写**逐项**或**整体**（**D-13**、**D-28**）。
8. **说明面板（画布下）**：**D-22**、**D-23**、**D-18**；**Esc** **D-11**。
9. **复制提示词**：变更后 **D-08** 自动写剪贴板；说明层 **Enter** 提交时再立刻刷新并可有「已复制」类反馈（**D-23**）；**⌘/Ctrl+C** 同文替换；与 **D-15** 仅影响引导是否展示说明句。
...
- [ ] 单选：建立选取后为 **N**；按 **Enter**（**D-28**）进入 **逐项 O**（**D-13**）。
- [ ] 多选：**D-12** 形成期不打开 **O**；完成后 **Enter** 先 **整体** 再可 **逐项**（**D-13**）；**D-17** 主框与整体作用域。
- [ ] 改选后默认 **N**，直至 **Enter** 或 **编辑说明** / 等价角标（**D-28**）。
- [ ] **D-01**/**D-02**/**D-19** 描边；**D-20** 元素轮廓与 **D-17** 包络上均有 **Enter** 提示角标；打开路径符合 **D-13**/**D-28**；页上若有移除控件则 **D-05**/**D-21**；**D-30** 键盘移除/清空。
...
- [ ] 用语 **复制提示词**（**D-06**）。
- [ ] Esc **D-11**；空选 **D-10**。
- [ ] **D-18** / **D-23** 无完成清除关闭按钮；**D-28** 说明关时 **Enter** 开层（**A** 且宿主非可编辑区外）；层内 **Enter** 提交；Ctrl+Delete（及 macOS ⌘ 组合）/ 扩展 UI 外 pointerdown 行为符合 PRD。
...
| 从 | 事件 | 到 |
|----|------|-----|
| E + A + N | 首次单击唯一元素，或范围松手仅一项 | H + A + N（**D-28**，待 **Enter**） |
| E + A + N | 范围松手 **≥2** 项 | H + A + N（**D-12**；待 **Enter** 开整体，**D-28**） |
| H + A + N | **Enter**（**D-28**：单选或整体已放行后的逐项） | H + A + O（**逐项**） |
| H + A + N | **Enter**（**D-28**：**≥2** 且整体闸未放行） | H + A + O（**整体**） |
| H + P + N | **Enter** | 不打开 **O**（**D-19**；须 **Space** 回 **A** 后再 **D-28**） |
| H + * + O | 改选焦点到另一对象 | H + * + N |
| H + * + N | 点「编辑说明」或等价 **Enter** 角标 | H + * + O（层别依 **D-13**） |
| H + * + * | 从选取去掉一项（**D-30** 键盘；页上控件若保留则 **D-21**） | 更新 H；无剩余则 E + A + N（**D-10**） |
| H + * + O | **Esc** | H + * + N（**D-11**） |
| H + * + N | **Esc** | E + A + N（**D-11** → **D-10**） |
| E／H + A + * | **Space** | 对应 P；再按回 A |

**Shift** 追加至 **≥2** 项且序列结束（**D-12**）→ **H+A+N**，再 **Enter** → **整体 O**（**D-28**）。**Esc**：**O** 下先关板再 Esc 清空（**D-11**）；**N** 且未连续退出或标志过期 → **单次 Esc** 清空。
