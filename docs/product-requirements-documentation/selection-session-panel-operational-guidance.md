...
- **呈现**：**整体修改说明** → 界面「**对当前选取的说明**」；**逐项修改说明** → 「**修改说明**」。角标 **编辑说明** 进入路径不变。**D-18** / **D-23**：无「完成」「清除」「关闭」控件；**Enter / 换行** 等短提示在输入层内展示（**不**展示 Ctrl+Delete 清空正文句），主面板仅保留极简状态句。
...
| D-23 | 说明编辑默认逻辑 | **Enter**（不含 Shift）提交：整体闸未放行时等同**完成整体说明**并关闭；其余为**关闭说明层**（草稿随输入实时写入）。**Delete / Backspace** 逐字删；**Ctrl+Delete** / **⌘+Delete** / **⌘+Backspace** 仍清空该层正文（**不**在输入层提示行展示该快捷键）。**Enter / Shift+Enter 换行** 的短提示置于**输入浮动层内**（输入框下沿一行），**「提示」** Tab **不**重复长句。扩展 UI 外 **pointerdown** 关闭说明层等仍适用。 |
